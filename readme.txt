...
Step 10: PinochleGame.h/cpp: We added an enum class PinochleMelds to the header file that contains all the different melds that pinochle can have. In the source file, we created an array that we can index into by casting an object of that enum class to an int to get its appropriate point value.
